...
test("write-release-attestation-runtime-report succeeds for valid fixtures", async () => {
  ...
  try {
    ...
    assert.equal(report.package.online.attestationCount, 1);
    assert.equal(report.package.offline.attestationCount, 1);
    assert.equal(report.oracleLock.online.attestationCount, 1);
    ...
    assert.deepEqual(report.package.online.failures, []);
    assert.deepEqual(report.package.offline.failures, []);
    assert.deepEqual(report.oracleLock.online.failures, []);
...
test("write-release-attestation-runtime-report fails deterministically for invalid lock fixture", async () => {
  ...
  try {
    ...
    assert.equal(report.oracleLock.online.ok, false);
    assert.equal(report.oracleLock.offline.ok, true);
    assert.ok(
      report.oracleLock.online.failures.includes("record[0]: sourceRepositoryRef mismatch")
    );
    assert.ok(
      report.oracleLock.online.failures.includes("record[0]: sourceRepositoryDigest mismatch")
    );
    assert.ok(
      report.oracleLock.online.failures.includes("record[0]: subject[0] missing sha256")
    );
    assert.ok(
      report.oracleLock.online.failures.includes("no subject matched expected identity")
    );
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
...
test("write-release-attestation-runtime-report fails when expected package digest mismatches", async () => {
  ...
  try {
    ...
    assert.ok(
      report.package.online.failures.includes("no subject matched expected digest")
    );
    assert.ok(
