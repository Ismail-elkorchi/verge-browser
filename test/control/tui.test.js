...
import { documentLayout, documentScrollRow, scrollToSource } from "../../dist/ui/document-layout.js";
...
test("interactive browser renders fragment rows and preserves a link focus identity across resize", async () => {
  ...
  try {
    ...
    assert.equal(documentLayout(document, 71, 21), documentLayout(document, 71, 21));
...
test("terminal control focus reveals offscreen fragment geometry through the page viewport", async () => {
  ...
  try {
    ...
    const layout = documentLayout(current, 71, 21).fragments;
...
test("outline node references resolve through container fragment geometry", async () => {
  ...
  try {
    ...
    assert.ok(heading);
    const layout = documentLayout(document, 79, 24).fragments;
...
test("summary activation updates document state and reveals details through the same pipeline", async () => {
  ...
  try {
    ...
    assert.match(documentLayout(opened, 80).fragments.rows.map((row) => row.text).join("\n"), /Secret text/u);
  } finally {
    await runtime.dispose();
    await prepared.controller.close();
  }
...
test("terminal-ui form controls update document state and submit through semantic form metadata", async () => {
  ...
  try {
    ...
    assert.ok(documentLayout(initial, 80).fragments.focusTargets.some((target) => target.node === query.node));
    await runtime.dispatch({
      kind: "formText",
...
test("standalone controls use the same terminal-ui editing path without inventing a form", async () => {
  ...
  try {
    ...
    assert.ok(documentLayout(initial, 80).fragments.focusTargets.some((target) => target.node === control.node));
    await runtime.dispatch({
      kind: "formText",
...
test("one-shot output and the interactive view consume the same fragment row text", async () => {
  ...
  try {
    const presentation = documentLayout(runtime.state().documents[0], 79).fragments;
    assert.ok(presentation.rows.some((row) => row.text.includes("Index")));
