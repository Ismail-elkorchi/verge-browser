...
- Node/npm exposes the supported library API; the CLI’s workspace controller,
  persistent store, and browser component models remain implementation details.
- Shared concepts such as URL resolution and protocol safety are behaviorally
  aligned across both entrypoints.
- CLI-specific component models, command help, browser sessions, and Node
  platform services are npm/Node concerns, not part of the published JSR API.
