...
| Unbounded placeholder promotion | One scheduler accounts for three live loads (one foreground reservation and two background slots) and a 256-job queue. Fifty-selection and queued/live-close barriers prove capacity and cleanup ownership. |
...
| Logical matches retain physical rows | Matches carry document/state/query/request identities; layout anchors are separate. Resize reprojects unchanged matches. Control edits, same-query reopen, obsolete completion, navigation, and inactive-tab controls reject stale anchors. |
